public static class Constants
{
    public const string SuccessfullRegistration = "Successfully registered {0}";
    
    public const string ProvidersRepaired = "Providers are repaired by {0}";

    public const string OreOutputToday = "Produced {0} ore today!";

    public const string EnergyProducedToday = "Produced {0} energy today!";

    public const string ModeChanged = "Mode changed to {0}!";

    public const string NoEntityFoundWithGivedId = "No entity found with id - {0}";

    public const string UnknownCommandType = "Unknown command type: {0}";

    public const string UnknownProvider = "Unknwn provider: {0}";

    public const string UnknownHarvester = "Unknwn harvester: {0}";

    public const string SystemShutdown = "System Shutdown";

    public const string TotalEnergyProduced = "Total Energy Produced: {0}";

    public const string TotalMinedOre = "Total Mined Plumbus Ore: {0}";
}